/**
 * Please note that we have not yet released back‑propagation code for the fisheye version; 
	...
	float* dL_dcov)
{
	...
	const float x_grad_mul = 1; // txtz < -limx || txtz > limx ? 0 : 1;
	const float y_grad_mul = 1; // tytz < -limy || tytz > limy ? 0 : 1;
	...
	float focal_x, float focal_y)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float ddelx_dx = 0.0f;
			const float ddely_dx = 0.0f;
			const float ddelx_dy = 0.0f;
			const float ddely_dy = 0.0f;
			/**
			 * --------------------------------------------------------------------------------------------------------------------------------------------------------
			 * --------------------------------------------------------------------------------------------------------------------------------------------------------
			 * ----------------------------------------------------- TODO (Not Implemented yet) -----------------------------------------------------------------------
