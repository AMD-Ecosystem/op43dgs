 ...
 * Compared to the bounding box, the correctness of the function values is more important. (for pinhole camera only)
*/
	...
	bool prefiltered)
{
	...
	//  some other algorithms may be used here to optimize bounding-box calculation.
	...
	// The distance between two points may be more suitable for depth sorting compared to the projection of the distance along the z-axis.
	depths[idx] = p_view.z; // sqrt(p_view.x * p_view.x + p_view.y * p_view.y + p_view.z * p_view.z);  
